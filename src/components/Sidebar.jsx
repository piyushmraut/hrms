...
const Sidebar = () => {
  ...
  return (
    ...
    >
      {/* Logo and Toggle button combined */}
      <div className={`p-4 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} transition-opacity duration-500 ${loaded ? 'opacity-100' : 'opacity-0'}`}>
        <div className="flex items-center justify-center bg-white bg-opacity-20 rounded-xl p-2 backdrop-blur-sm transition-all duration-300 hover:scale-105">
          <span className="text-cyan-300 text-2xl font-bold">+</span>
          {!isCollapsed && <span className="ml-2 text-white font-bold transition-all duration-300">Efficio</span>}
        </div>
        ...
        <button 
          className={`w-6 h-6 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-md flex items-center justify-center cursor-pointer shadow-md transition-transform hover:scale-105 ${isCollapsed ? 'mx-auto mt-2' : ''}`}
          onClick={toggleSidebar}
        >
          <svg 
            className={`w-4 h-4 text-white transform transition-transform duration-300 ${isCollapsed ? 'rotate-0' : 'rotate-180'}`} 
